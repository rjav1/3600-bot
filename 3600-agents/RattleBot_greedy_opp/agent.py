"""RattleBot v0.7 (greedy_opp fork) — BS-2 greedy-opp model on v0.4.2.

This is the `RattleBot_greedy_opp` fork (phase2-shipper, 2026-04-18)
built on top of v0.4.2 (BS-3 carpet penalty + opening-PRIME hardening).
The only functional delta vs v0.4.2 lives in `search.py`:

- v0.7 BS-2 (search.py): when `SEARCH_ASSUME_GREEDY_OPP=True`, opp-to-
  move nodes in `_alphabeta` skip negamax enumeration and descend into
  ONE greedy opp reply chosen by
  `immediate_delta + 0.25 * cell_potential_for_opp`. Branch factor at
  opp plies drops from ~7 to 1, yielding ~7× effective depth from the
  same compute. Rationale per CONTRARIAN_APR18.md §1 BS-2: real
  student opponents and Carrie play fixed-heuristic greedy, not
  minimax against us, so pure-negamax was verifying against a straw
  man (97.9% cutoff-on-first-move).

All v0.4.2 shipped fixes are inherited unchanged:
...
class PlayerAgent:
    ...
    def commentate(self) -> str:
        ...
        return (
            "RattleBot v0.7 (greedy_opp) — alpha-beta + ID + HMM belief + BS-2 greedy-opp "
